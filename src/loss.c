...
internal void
gz_loss_binary_cross_entropy_(t32 *A, t32 *B, t32 *Result, reduce_method ReduceMethod) {
    ...
    /* NOTE(Abid): In case we are red_Mean or red_Sum */
    *((f32 *)Result->Data.Ptr + ResultOffset) = 0;
    ...
    for(size_t OpNum = 1; OpNum <= ExpectedNumOps; ++OpNum) {
        ...
        /* NOTE(Abid): If we do not have red_None, then we will discard any value inside result,
    ...
    if(ReduceMethod == reduce_mean) *((f32 *)Result->Data.Ptr) = *((f32 *)Result->Data.Ptr) / ExpectedNumOps;
}

inline internal t32 *
gz_loss_binary_cross_entropy(t32 *a, t32 *b, reduce_method reduce_method, mem_arena *arena) {
    t32 *result;
    if(reduce_method == reduce_none) {
    ...
    gz_loss_binary_cross_entropy_(a, b, result, reduce_method);
    return result;
}
